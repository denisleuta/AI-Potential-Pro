...
def compute_category_scores(
    ...
    for cat, metrics in categories_def.items():
        ...
        positive_score = (
            df_norm[positive_metrics].mean(axis=1) if positive_metrics else 0
        )
        negative_score = (
            df_norm[negative_metrics].mean(axis=1) if negative_metrics else 0
        )

        df_norm[cat] = positive_score / (positive_score + ((negative_score)/2))
...
def main():
    ...
    for pos in positions:
        ...
        ranking = get_rankings(df_pos, "overall_index", top_n=100)
